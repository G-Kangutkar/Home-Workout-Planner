...
//     // Return as local datetime string — Google will use the timeZone field
//     const [hours, minutes] = timeStr.split(":");
//     return `${year}-${month}-${date}T${hours}:${minutes}:00`;  // no Z = not UTC
// }
...
//             const [startH, startM] = (profile.preferred_workout_time || "07:00:00").split(":").map(Number);
//             const endMinutes = startH * 60 + startM + durationMin;
//             const endH = String(Math.floor(endMinutes / 60) % 24).padStart(2, "0");
//             const endM = String(endMinutes % 60).padStart(2, "0");
//             const datePart = startTime.split("T")[0]; // get YYYY-MM-DD from startTime
...
//                     colorId,
//                     start: {
//                         dateTime: startTime,           // no .toISOString() — keep as local string
//                         timeZone: "Asia/Kolkata",      // ← tell Google this is IST
//                     },
//                     end: {
//                         dateTime: endTime,             // see below
...
//                 const mealTimeStr = mealTime.slice(0, 5); // "08:00"
//                 const startTime = buildDateTime(dayOffset, mealTimeStr);
//                 const [h, m] = mealTimeStr.split(":").map(Number);
//                 const endMinutes = h * 60 + m + 30; // 30 min meal prep
//                 const endH = String(Math.floor(endMinutes / 60) % 24).padStart(2, "0");
//                 const endM = String(endMinutes % 60).padStart(2, "0");
//                 const datePart = startTime.split("T")[0];
//                 const endTime = `${datePart}T${endH}:${endM}:00`;// 30 min meal prep
